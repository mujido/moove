...
for p in (args[2])
  ...
  elseif (is_player(p))
    listing = {@listing, tostr(p:title(), " (", p, ") ", t == E_PROPNF ? "is not a $player." | "has a garbled .last_disconnect_time.")};
  else
    listing = {@listing, tostr(p:title(), " (", p, ") is not a player.")};
  endif
endfor
for p in (args[1])
  if (p in offs)
  ...
  elseif (is_player(p))
    listing = {@listing, tostr(p:title(), " (", p, ") not logged in.", t == E_PROPNF ? "  Not a $player." | "  Garbled .last_disconnect_time.")};
  else
    listing = {@listing, tostr(p:title(), " (", p, ") is not a player.")};
  endif
endfor
...
for lst in ({@idles, @offs})
  ...
  p = lst[3];
  "p:title() and this:ansi_title(p) should be the same length, saves a call to this:length";
  namestr = tostr(this:cutoff(this:terminate_normal(this:ansi_title(p)), 1, min(max_name, z = length(p.name)), 1), " (", p, ")");
  name_width = max(this:length(namestr), name_width);
  names = {@names, namestr};
  typeof(wlm = p.location:who_location_msg(p)) == STR || (wlm = valid(p.location) ? p.location:title() | tostr("** Nowhere ** (", p.location, ")"));
  locations = {@locations, this:terminate_normal(wlm)};
endfor
...
return total;
"Last changed by Merlin (#95) on Sat Mar  4 14:41:34 2000 CST";
"Last changed by Sorcerer_X, #95@Mithreal, on Wed Apr 25 12:45:49 2001 CDT";
